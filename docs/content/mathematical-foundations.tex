...
We can see that these are equivalent by writing out $\text{grad}(\text{div}(f))
- \text{curl}(f)$ explicitly:
...
Another useful identity that will come up later on (\todo{ref. section-...}) is
$$\curl(\curl{\vb{u}}) \equiv \grad(\div{\vb{u}}) - \div{\grad{\vb{u}}}.$$

\section{Optimization}\label{section:optimization}
We address optimization scenarios where the target is to minimize a function
with respect to one of its inputs:


\todo{Loss Function}

\todo{Gradient Descent, basic, straightforward, first-order accurate method}

\todo{Mention other optimization methods (second-order accurate), Newton's
method, etc...}

\todo{Refer to Hessian, and also write that the Laplacian is
$\text{trace}(\text{Hessian})$?}
...
\begin{equation}\label{eq:NN-min}
    ...
    = \arg\min_{\boldsymbol\theta} 
        \left| \vb{f}(\vb{x}, \boldsymbol\theta) - \vb{y}^* \right|_2^2.
\end{equation}

As already discussed in section~\ref{section:optimization}, we can use the
...
$$\vb{o}^i = \sigma(\vb{W}_i \vb{o}^{i-1} + b_i),$$
...
respectively. We call every $\vb{W}_i$ and $b_i$ the parameters of the \ac{NN},
and collect their values in $\boldsymbol\theta$.
...
\todo{Is this section about NNs too wordy?}
