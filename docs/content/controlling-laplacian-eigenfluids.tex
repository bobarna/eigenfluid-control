...
\todo{Would it make sense to write any of these as an Algorithm? It would be
nice to refer back to them as Algorithm X later on.}
...
In this scenario, we optimize for a force vector $\vb{f} \in
...
\todo{Formulate the NN as a function, write what we are optimizing for.}
...
We overfit the \ac{NN} to a single training sample. Plotting the results of
...
Using $N=16$ basis fields, we train a \ac{NN} for the same control force
estimation problem as detailed above. 
